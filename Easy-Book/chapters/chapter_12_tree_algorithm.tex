...
\paragraph{Summary of Recurrence Relation} Due to the fact that divide-and-conquer is inherent to the recursive structure of trees and root-children relation; a binary tree of size $n$ with root will be divided into three non-overlapping parts: left-subtree, root, right-subtree. %The problem of $n$ is thus divided into $d(t) = f(d(t_l), d(t_r), t)$, and there has no overlapping between these three parts. 
